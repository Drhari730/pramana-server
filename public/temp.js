...
function renderSearchResults(){
  ...
  el.innerHTML=`<div class="row" style="margin:14px 0 10px">
    ...
    ${SEARCH_RESULTS.slice(0,40).map(r=>`<div class="study" style="padding:11px 14px">
      <div class="st-title" style="font-size:13.5px">${highlightKw(r.title)}</div>
...
/* ===================== SCREENING ENGINE ===================== */
let SCREEN_FILTER='all', SCREEN_Q='', _incReg=null, _excReg=null;
...
function filteredRefs(){
  ...
  if(SCREEN_FILTER==='pending')refs=refs.filter(r=>!visibleStageDecision(r,'ta'));
  else if(SCREEN_FILTER==='include'||SCREEN_FILTER==='exclude'||SCREEN_FILTER==='maybe')refs=refs.filter(r=>(visibleStageDecision(r,'ta')||{}).final===SCREEN_FILTER);
  else if(SCREEN_FILTER==='conflict')refs=refs.filter(r=>r._conflict);
  if(SCREEN_Q)refs=refs.filter(r=>((r.title||'').toLowerCase()+(r.abstract||'').toLowerCase()).includes(SCREEN_Q.toLowerCase()));
  return refs;
}
...
function setHighlightWords(){
  ST.settings.incWords=(document.getElementById('kw-inc')||{}).value;
  ST.settings.excWords=(document.getElementById('kw-exc')||{}).value;
  _incReg=ST.settings.incWords?new RegExp('('+ST.settings.incWords.split(',').map(w=>escRegex(w.trim())).join('|')+')','gi'):null;
  _excReg=ST.settings.excWords?new RegExp('('+ST.settings.excWords.split(',').map(w=>escRegex(w.trim())).join('|')+')','gi'):null;
  saveSoon();renderScreenList();
}
function highlightKw(text){
  let s=esc(text);
  if(_incReg)s=s.replace(_incReg,'<mark style="background:#dcfce7;color:#166534">$1</mark>');
  if(_excReg)s=s.replace(_excReg,'<mark style="background:#fee2e2;color:#991b1b">$1</mark>');
  return s;
}
function onSearchInput(q){SCREEN_Q=q;renderScreenList();}
function setScreenFilter(f){SCREEN_FILTER=f;renderScreen();}
...
function renderScreen(){
  ...
  <div class="toolbar" style="margin-bottom:14px; gap: 8px;">
    <div class="search-box" style="flex:1"><input id="kw-inc" placeholder="Highlight green (e.g. RCT, adult)" value="${esc(ST.settings.incWords||'')}" onchange="setHighlightWords()"></div>
    <div class="search-box" style="flex:1"><input id="kw-exc" placeholder="Highlight red (e.g. animal, review)" value="${esc(ST.settings.excWords||'')}" onchange="setHighlightWords()"></div>
  </div>
  <div class="toolbar">
    ...
    <div class="search-box"><input placeholder="Search title, abstract, text..." value="${esc(SCREEN_Q)}" oninput="onSearchInput(this.value)"></div>
  </div>
...
function localTrainingLabels(){
  ...
  ST.refs.filter(r=>!r.dup).forEach(r=>{
    ...
    if(r.reviews && r.reviews.ta) {
      const reviewers = Object.keys(r.reviews.ta);
      if(reviewers.length > 0) {
        const teamFinal = r.reviews.ta[reviewers[0]].final;
        if(['include','exclude'].includes(teamFinal)){rows.push({r,label:teamFinal});return;}
      }
    }
...
function filteredRefs(){
  ...
  if(SCREEN_Q){const q=SCREEN_Q.toLowerCase();u=u.filter(r=>(r.title+' '+r.authors+' '+(r.abstract||'')+' '+(r.ft&&r.ft.pdfText||'')).toLowerCase().includes(q));}
...
window.setHighlightWords=setHighlightWords;
function highlightKw(text){
  if(!text) return '';
  let res = esc(text);
  if(ST.settings.incWords){
    ST.settings.incWords.split(',').map(s=>s.trim()).filter(Boolean).forEach(w=>{
      res = res.replace(new RegExp('('+escRegex(w)+')', 'gi'), '<mark style="background-color:#d1fae5;color:#065f46;font-weight:bold">$1</mark>');
    });
  }
  if(ST.settings.excWords){
    ST.settings.excWords.split(',').map(s=>s.trim()).filter(Boolean).forEach(w=>{
      res = res.replace(new RegExp('('+escRegex(w)+')', 'gi'), '<mark style="background-color:#fee2e2;color:#991b1b;font-weight:bold">$1</mark>');
    });
  }
  return res;
}
...
function studyCard(r){
  ...
  return`<div class="study ${dec}" data-id="${r.id}">
    <div class="st-title">${highlightKw(r.title)}</div>
    ...
    ${r.abstract?`<div class="st-abs" onclick="this.classList.toggle('open')">${highlightKw(r.abstract)}</div>`:'<div class="muted">No abstract - consider fetching it in Full text.</div>'}
    <div class="st-foot">
...
async function serverTeamSplitModal(){
  ...
  const progressRows=progress.map(p=>'<tr><td><b>'+esc(p.name||'')+'</b><div class="muted">'+esc(p.email||'')+'</div></td><td>'+p.assigned+'</td><td>'+p.reviewed+'</td><td>'+p.pending+'</td><td>'+p.counts.include+'/'+p.counts.maybe+'/'+p.counts.exclude+'</td><td>'+(editable&&p.pending>0?'<button class="btn sm" onclick="remindReviewer(\''+esc(p.email)+'\')">Remind</button>':'')+'</td></tr>').join('');
  ...
  '<div class="modal-b">'+
    ...
    '<div style="overflow-x:auto"><table class="tbl"><thead><tr><th>Reviewer</th><th>Assigned</th><th>Reviewed</th><th>Pending</th><th>I/M/E</th><th>Action</th></tr></thead><tbody>'+(progressRows||'<tr><td colspan="6">No reviewer progress yet.</td></tr>')+'</tbody></table></div>'+
...
async function remindReviewer(email){
  if(!confirm('Send reminder email to '+email+' to complete screening?'))return;
  toast('Sending...');
  try{
    const r=await req('/api/projects/'+ST.id+'/remind',{method:'POST',body:JSON.stringify({email})});
    if(r.ok)toast('Reminder sent','ok');
    else toast(r.error||'Failed','err');
  }catch(e){toast(e.message,'err');}
}
window.remindReviewer=remindReviewer;
